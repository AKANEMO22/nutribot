...
def build_fpt_project(project_dir: Path) -> tuple[bool, str]:
    ...
    if npm_path is None:
        return False, "Khong tim thay npm trong PATH, khong the auto-build FPT project."
    ...
    if result.returncode != 0:
        ...
        return False, f"Auto-build FPT that bai. Chi tiet: {error_output}"

    return True, "Auto-build FPT thanh cong."
...
def source_build_is_newer(source_build_dir: Path, embedded_build_dir: Path) -> bool:
  source_index = source_build_dir / "index.html"
  embedded_index = embedded_build_dir / "index.html"
  if not source_index.exists():
    return False
  if not embedded_index.exists():
    return True
  return source_index.stat().st_mtime > embedded_index.stat().st_mtime
...
def resolve_fpt_build_dir() -> tuple[Optional[Path], str]:
  ...
  if project_dir is not None:
    ...
    if source_build_is_newer(source_build_dir, EMBEDDED_BUILD_DIR):
      ok, message = sync_build_to_embedded(source_build_dir)
      if not ok:
        return None, message
      return EMBEDDED_BUILD_DIR, message
